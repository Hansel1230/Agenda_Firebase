 import { insert, insertContactos, getUserInfo} from "./firestore.js";
...
function saveInfo(key, value)
{
    ...
    // Agregar el arreglo a Firebase
}


async function saveInfoFireBase(obj)
{
    try{
        const response = await insertContactos({...obj},obj.userEmail);
    } catch (error) {
        console.error(error);
    }
...
if (window.location.pathname.split("/").pop() == "index.html") {
    document.querySelector('#boton_Sing_Up').addEventListener('click',login_Sing_Up);
    document.querySelector('#boton_Sing_In').addEventListener('click',login_Sing_In);
}
...
if (window.location.pathname.split("/").pop() == "CRUD.html") {
    document.body.addEventListener('load',showContacts())
...
function showContacts()
{
    ...
    user.contacts.forEach(contact => {
        ...
        parrafo.textContent=`Nombre: ${contactName} -  Apellidos: ${contactLastName} -  Email: ${contactEmail} -  Número: ${contactPhoneNumber}`
...
if (window.location.pathname.split("/").pop() == "email-handler.html") {
